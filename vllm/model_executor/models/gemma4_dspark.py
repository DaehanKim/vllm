...
A dense Gemma4 backbone reused as a DFlash-style draft: the target's auxiliary
hidden states (at ``target_layer_ids``) are combined via ``fc`` into a per-layer
attention context, that context's K/V is pre-inserted into each draft layer's
cache (``precompute_and_store_context_kv``), and a non-causal block of
``num_speculative_tokens`` query tokens is run in one parallel pass. DSpark then
injects intra-block dependency with the sequential Markov head, exactly as for
Qwen3 DSpark (see ``qwen3_dspark.py`` / the generic ``DSparkSpeculator``).

Differences from the Qwen3 DSpark backbone (``qwen3_dflash.py``):
  * Gemma4 layer internals: sandwich norms (input / post-attn / pre-ffn /
    post-ffn) + per-layer ``layer_scalar``, gelu-tanh gated MLP, per-head
    Q/K/V RMSNorm, ``scaling = 1.0``, partial rotary, ``global_head_dim``.
  * ``attention_k_eq_v``: full-attention layers have no ``v_proj`` — V is taken
    from the ``k_proj`` output (V = v_norm(k_proj(x)), no RoPE), while
    K = RoPE(k_norm(k_proj(x))). This matches the checkpoint, which ships only
    ``q_proj`` / ``k_proj`` per layer.
  * ``final_logit_softcapping`` applied on the base logits.

Self-contained checkpoint (own ``embed_tokens`` / ``lm_head``, ``tie=False``);
``dspark_shares_target_embeddings = False`` so the loader keeps these weights.
The DSpark Markov / confidence heads are shared with the Qwen3 DSpark model.
"""
